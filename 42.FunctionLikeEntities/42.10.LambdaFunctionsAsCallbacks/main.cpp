...
std::string & modify(std::string& str_param,
                     char(* modifier)(const char&))                          // Callback 1 parameter is here
{
...
BoxContainer<std::string>& modify(BoxContainer<std::string>& sentence,
                                  char(*modifier) (const char&)){            // Callback 2 parameter is here
...
std::string get_best (const BoxContainer<std::string>& sentence,
                      bool(*comparator)(const std::string& str1,             // Callback 3 parameter is here
...
int main(){
    ...
    std::cout << "Encrypted : " <<  modify(str,encrypt) << std::endl;        // Lambda as Callback-1-argument
    std::cout << "Decrypted : " << modify(str,decrypt) << std::endl;         // Lambda as Callback-1-argument

    std::cout << "---------" << std::endl;

    //Passing lambda functions directly
    std::cout << "Initial : " << str << std::endl;
    std::cout << "Encrypted : " <<  modify(str,[](const char& param){        // In-place lambda as Callback-1-argument
        return static_cast<char> (param + 3);
    }) << std::endl;
    std::cout << "Decrypted : " << modify(str,[](const char& param){         // In-place lambda as Callback-1-argument
        return static_cast<char> (param - 3);
    }) << std::endl;
    ...
    std::cout << "Encrypted : " << modify(quote,encrypt) << std::endl;       // Lambda as Callback-2-argument
    std::cout << "Decrypted : " << modify(quote,decrypt) << std::endl;       // Lambda as Callback-2-argument
   ...
   auto larger_in_size = [] (const std::string& str1, const std::string& str2){ //
    ...
    std::cout << "larger in size : " << get_best(quote,larger_in_size) << std::endl; // Lambda as Callback-3-argument

    std::cout << "greater lexicographicaly : " 
            << get_best(quote,greater_lexicographically) << std::endl; // Lambda as Callback-3-argument
