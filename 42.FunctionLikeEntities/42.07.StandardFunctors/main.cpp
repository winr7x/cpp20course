...
int main() {
    ...
    std::plus<int> adder;               // NEW_FOR_ME C++ Built-in functor
    std::minus<int> substracter;
    std::greater<int> compare_greater;
    
    std::cout << std::boolalpha;
    std::cout << " 10 + 7 : " << adder(10,7) << std::endl; //17 // NEW_FOR_ME using built-in functor
    ...
    std::cout << "greater string : " <<
            get_best(quote,string_comparator) << std::endl; // NEW_FOR_ME 2nd param is 'typename Comparator'. So we can pass built-in functor
    //Custom function pointer
    std::cout << "greater string : " 
            << get_best(quote,custom_greater<std::string>) << std::endl; // We can pass ordinar function
    //Custom functor
    Greater<std::string> greater_string_custom_functor;
    std::cout << "greater string : "
        << get_best(quote,greater_string_custom_functor) << std::endl; // We can pass functor
