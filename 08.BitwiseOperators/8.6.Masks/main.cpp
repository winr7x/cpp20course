...
int main(){
	...
	//SETTING BITS
    //Setting : |= with mask of the bit                                   // |= mask NEW_FOR_ME set specific bit to 1
    ...
    //Resetting : &= (~mask)                                               // &=(~mask) NEW_FOR_ME unset specific bit to 0
	...
	//Set all bits
	std::cout << "Setting all bits" << std::endl;                          // &=(mask1 | mask2) NEW_FOR_ME set multiple bits to 1 simultaneously
	...
	std::cout << "Reset bits at pos 0,2,4,6" << std::endl;                 // &=~(mask1 | mask2) NEW_FOR_ME unset multiple bits to 0 simultaneously
    ...
    var ^= mask_bit_0;                                                    // ^=mask1 NEW_FOR_ME toggle specific bit (1 --> 0, 0 --> 1)
	std::cout << std::setw(COLUMN_WIDTH) <<  "var : "
		 << std::setw(COLUMN_WIDTH) << std::bitset<8>(var) << std::endl;
    ...
    var ^= (mask_bit_7 | mask_bit_6 | mask_bit_5 | mask_bit_4);           // ^=(mask1 | mask2) NEW_FOR_ME toggle multiple bits simultaneously
	std::cout << std::setw(COLUMN_WIDTH) <<  "var : "
		 << std::setw(COLUMN_WIDTH) << std::bitset<8>(var) << std::endl;
