...
public:
	...
	// NEW_FOR_ME define a const iterator
	class ConstIterator 
    {
    public : 
        using iterator_category = std::random_access_iterator_tag;
        using difference_type   = std::ptrdiff_t;
        using value_type        = T;
        using pointer_type           = const T*; // NEW_FOR_ME way to define a const pointer
	...
	// NEW_FOR_ME these definitions are used when user iterate over a const container using begin(),end()
	// instead of cbegin(), cend()
	...
	// NEW_FOR_ME way to define cbegin(), cend() for const containers
