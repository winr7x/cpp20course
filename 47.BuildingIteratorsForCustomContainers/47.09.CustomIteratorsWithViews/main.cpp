...
// NEW_FOR_ME views is applied to custom container BoxContainer
...
int main(){
  ...
  std::cout << "vi evens : " ;
  print(std::ranges::filter_view(vi,evens)); // Print even numbers on the fly directly
#endif
  ...
  print(v_transformed);
  std::cout << "vi : ";
  print(vi); // Check that container has not been modified
#endif
  ...
  auto less_than_15_v = class_room | std::views::take_while([] (const Student& s) {
    return (s.m_age <15);
  });
  std::cout << "\nstudents under 15 : \n";
