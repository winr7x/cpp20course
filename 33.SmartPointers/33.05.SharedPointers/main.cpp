...
int main(){
    ...
    //shared pointers to fundamental types
    {
        ...
        *int_ptr_1 = 40; // Use the pointer to assign
        std::cout << "The pointed to value is : " << *int_ptr_1 << std::endl;
        std::cout << "Use count : " << int_ptr_1.use_count() << std::endl; // NEW_FOR_ME get reference counter
        ...
        *int_ptr_5 = 100;
        ...
        std::shared_ptr<int> int_ptr_my{int_ptr_1.get()}; // But this way of counting references does not work
        std::cout << "Use count for int_ptr_my : " << int_ptr_my.use_count() << std::endl; // Output: 1
        *int_ptr_my = 777;
        std::cout << "Use count for int_ptr_5 is still : " << int_ptr_5.use_count() << std::endl; // use_count has not changed
        std::cout << "*int_ptr_5 is still : " << *int_ptr_5 << std::endl; // value has not changed

        //Use a shared pointer to manage an already existing piece of memory
        //(To take over pointer)
        std::cout << "-----" << std::endl;
        ...
        std::cout << "Use count for int_ptr_1 : " << int_ptr_1.use_count() << std::endl; // Output: 4
        std::cout << "Use count for int_ptr_2 : " << int_ptr_2.use_count() << std::endl;
        std::cout << "Use count for int_ptr_3 : " << int_ptr_3.use_count() << std::endl;
        std::cout << "Use count for int_ptr_4 : " << int_ptr_4.use_count() << std::endl;
        std::cout << "Use count for int_ptr_5 : " << int_ptr_5.use_count() << std::endl;
        ...
        std::cout << "int_ptr_4 : " << int_ptr_4 << std::endl; // NEW_FOR_ME by default shared_ptr prints out address into cout like 0x2857a3c84a0
        ...
        if (int_ptr_4) { // NEW_FOR_ME way to check shared_ptr for not-nullptr. if (int_ptr_4.get()) is excess
                         // The same thing is for unique_ptr
   ...
   {
        // Too easy : behavior is similar to the behavior above
        // Initializing
