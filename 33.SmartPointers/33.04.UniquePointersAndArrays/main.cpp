...
int main(){
     ...
     {
        std::cout << std::endl;
        // NEW_FOR_ME way to use unique_ptr for array allocation
        [[maybe_unused]] auto arr_ptr_1 = std::unique_ptr<Dog[]> ( new Dog[3]{Dog("Dog7"), Dog("Dog8") , Dog("Dog9")} );

        auto arr_ptr_2 = std::make_unique<Dog[]>(3); // NEW_FOR_ME way to use make_unique for array allocation
                                                     // Drawback: Can't initialize individual elements
