...
int main() {
  ...
  std::ranges::sort(points, std::less<>{}, [](auto const &p) { // NEW_FOR_ME lambda here is a projection
    return p.m_x;                                              // element is passed into the lambda, then m_x is passed into the comparator
  });
  print_collection(points);
  ...
  print_collection(points);
  std::ranges::sort(points, std::less<>{}, &Point::m_y); // NEW_FOR_ME member variable as a projection
  print_collection(points);
