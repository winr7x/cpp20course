...
generator<int> generator_numbers()
{
...
generator<int> infinite_number_stream(int start = 0)
{
  auto value = start;
  for (int i = 0;;++i) {
    // std::cout << "In infinite_number_stream..." << std::endl;
    co_yield value;
    ++value;
  }
}
...
int main() {
  ...
  // std::cout << "color done : " << task1.m_handle.done() << std::endl;
