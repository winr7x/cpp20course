...
int main(){
    ...
    [[maybe_unused]] const auto insertion_result_pair = numbers.insert(14); // NEW_FOR_ME insert() returns pair<iterator, bool> with result about the insert operation
    ...
    auto result_emplace = numbers.emplace(13);  // Fails // NEW_FOR_ME emplace() returns pair<iterator, bool> with result about the insert operation
    ...
    [[maybe_unused]] std::set<int> numbers_01 {11,16,2,9,12,15,6,15,2};  // Use default functor (std::less)
    [[maybe_unused]] std::set<int,std::less<int>> numbers_02 {11,16,2,9,12,15,6,15,2};  // NEW_FOR_ME Built in functor
    [[maybe_unused]] std::set<int,std::greater<int>> numbers_03 {11,16,2,9,12,15,6,15,2}; // Built in functor
    [[maybe_unused]] std::set<int,IntComparator> numbers_04 {11,16,2,9,12,15,6,15,2}; // NEW_FOR_ME Use Custom functor
                     std::set<int,bool(*)(int,int)> numbers_05(compare_ints) ; //Function pointer
                     numbers_05.insert({11,16,2,9,12,15,6,15,2});
                    std::set<int, std::function<bool(int,int)>> numbers_06(   [](int left,int right){   // NEW_FOR_ME Lambda function
                                                                              return left > right;});   // Constructor is used: explicit set( const Compare& comp, const Allocator& alloc = Allocator() );
                    numbers_06.insert({11,16,2,9,12,15,6,15,2}); 
 
   
    std::cout << " numbers_01 :[ ";  for (const auto& elt : numbers_01) { std::cout << " " << elt ; } std::cout << "]\n";
    std::cout << " numbers_02 :[ ";  for (const auto& elt : numbers_02) { std::cout << " " << elt ; } std::cout << "]\n";
    std::cout << " numbers_03 :[ ";  for (const auto& elt : numbers_03) { std::cout << " " << elt ; } std::cout << "]\n";
    std::cout << " numbers_04 :[ ";  for (const auto& elt : numbers_04) { std::cout << " " << elt ; } std::cout << "]\n";
    std::cout << " numbers_05 :[ ";  for (const auto& elt : numbers_05) { std::cout << " " << elt ; } std::cout << "]\n";
    std::cout << " numbers_06 :[ ";  for (const auto& elt : numbers_06) { std::cout << " " << elt ; } std::cout << "]\n";
