#include <iostream>
#include <forward_list>                      // NEW_FOR_ME one-directional list
...
int main(){
    ...
    print_collection(numbers);
    auto it_insert = numbers.before_begin(); // NEW_FOR_ME forward_list has begin() but doesn't have insert(). Only insert_after()
    ...
    //Erase the 14
    numbers.erase_after(it_erase); // NEW_FOR_ME erase one element after iterator
    ...
    std::forward_list<int> numbers2 {2,4,6};
    ...
    //merge
    numbers1.merge(numbers2); // NEW_FOR_ME merges two sorted lists into one.
                              ...
                              // If not sorted then crash can happen
    ...
    numbers1.splice_after(           // NEW_FOR_ME Moves elements from another forward_list numbers2 (before_begin, cend) to numbers1 (before_begin).
        numbers1.before_begin(),
        numbers2,
        numbers2.before_begin(),
        numbers2.cend());
    ...
    numbers.remove(1); // NEW_FOR_ME remove both elements (equal to 1)
    ...
    numbers.unique(); // NEW_FOR_ME Removes contiguous duplicates: 100 100 will become 100, 11 11 will become 11
    ...
    // numbers.rbegin() // UNCOMMENT_TO_SEE It is impossible to traverse a singly linked list in reverse order since each
                        // node only points to the next node, and not to the previous node.
