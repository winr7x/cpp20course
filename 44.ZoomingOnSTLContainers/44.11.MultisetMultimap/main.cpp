#include <iostream>
#include <set> // NEW_FOR_ME there is no <multiset>
#include <map> // NEW_FOR_ME there is no <multimap>
...
int main(){
    ...
    std::multiset<int> numbers = {1,2,1,6,2,8,3,9,4,24,6,2}; // It has two '1', three '2', two '6'
    
    std::multiset<Book> books = { // it has two 1734-key books
        Book(1734,"Cooking Food"),
        Book(1930,"Building Computers"),
        Book(1734,"Farming for Beginners")};
           
    std::cout << "numbers : ";
    print_multiset(numbers); // Still is sorted
    
    std::cout << "books : " ;
    print_multiset(books); // Still is sorted
    
    std::cout << std::endl;
    std::cout << "multimap : " << std::endl;
    
    
    std::multimap<int,std::string> office_numbers = { // It has two 101-key numbers, three 102-key numbers
    ...
    print_multimap(office_numbers); // Still is sorted
    
    
    std::multimap<int,Book> other_books = { // It has two 1-key books
    ...
    print_multimap(other_books); // Still is sorted
