...
int main(){
    ...
    // numbers.insert(it_pos,400); // UNCOMMENT_TO_SEE crash in MSVC 17.5.1
                                // As we insert new items, it_pos changes the elements it's
    ...
    it_pos = numbers.begin() + 2; // NEW_FOR_ME cure the iterator from invalidated state
    ...
    items.erase(items.begin() + 4); // erase Item[value:9]
    
    print_collection(items);
    
    //Range : visualize
    items.erase(items.begin() + 1, items.begin()+4); // erase 3 elements: Item[value:7] Item[value:450] Item[value:8]
    ...
    items.emplace(items.end() , 10,10); // NEW_FOR_ME items.emplace(items.end(), ... )
