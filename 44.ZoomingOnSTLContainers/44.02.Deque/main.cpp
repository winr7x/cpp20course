...
int main(){
    ...
    // std::cout << "numbers[30] (Undefined behavior):" <<  numbers[30] << std::endl;// No bounds check, UNCOMMENT_TO_SEE undefined behavior, junk value or even crash.
    try {
        numbers.at(30);
    } catch(const std::exception& e) {
        std::cerr << "numbers.at(30) exception: " << e.what() << '\n';
    }
    ...
    // NEW_FOR_ME Returns the maximum number of elements the container is able to hold due to system or library implementation limitations,
    ...
    print_collection(numbers);
    // numbers.insert(it_pos,400); // UNCOMMENT_TO_SEE iterator invalidation and crash on MSVC 17.5.1
                                   // NEW_FOR_ME insert() invalidates iterators for deque: see https://en.cppreference.com/w/cpp/container : Iterator invalidation section
    // print_collection(numbers);
