...
public:
	...
	void invalidate(){
		std::cout << "Invalidating object: m_items = nullptr; m_size = 0; m_capacity = 0\n";
		m_items = nullptr;
		m_size = 0;
		m_capacity = 0;
	}
...
{
	std::cout << "BoxContainer main constructor called. capacity=" << capacity << '\n';
...
{
	std::cout << "BoxContainer object with size : " << m_size  <<" destroyed (destructor)" << std::endl;
	delete[] m_items;
}
...
template <typename T>
void BoxContainer<T>::operator =( BoxContainer<T>&& source){ // Move assignment operator
	std::cout << "BoxContainer move assignment operator=(BoxContainer<T>&&) called. Moving " 
			<< source.m_size << " items..." << std::endl;	

	if (this == &source)				// STEP 1: check for self assignment
            return;
			
	m_items = source.m_items;			// STEP 2: steal resources
	m_size = source.m_size;
	m_capacity = source.m_capacity;
	
	source.invalidate();				// STEP 3: invalidate temporary stolen object
