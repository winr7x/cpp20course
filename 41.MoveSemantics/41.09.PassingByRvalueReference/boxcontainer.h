...
void BoxContainer<T>::add( T&& item){
	...
	if (m_size == m_capacity)
		//expand(m_size+5); // Let's expand in increments of 5 to optimize on the calls to expand
		expand(m_size + EXPAND_STEPS);
	// m_items[m_size] = std::move(item); // NEW_FOR_ME again, since rvalue has name 'item' in this case, compiler treats 'item'as lvalue
	m_items[m_size] = std::move(item); // So std::move() is required
	++m_size;
}
